...
contract ROBOTS_OF_PIAIC is ERC721Enumerable, Ownable {
  using Strings for uint256;
  ...
  constructor(
  ) ERC721("ROBOTS_OF_PIAIC", "ROP") {
     
    setBaseURI("https://darkrise.win/metadata/");
  ...
  function mint(address _to, uint256 _mintAmount) public payable {
    ...
    if(msg.sender != owner()){
    require(!paused);
    require(block.timestamp <= saleEndTime , "The sale is over");
    }
  ...
  //set sale End timestamp
  function setSaleEnd(uint _time) public onlyOwner {
      saleEndTime = _time;
  }
